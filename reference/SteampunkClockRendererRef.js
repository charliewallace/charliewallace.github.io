...
class SteampunkClockRenderer {
  constructor(containerId = 'canvas-container', options = {}) {
    ...
    // Load Specialized Textures (User Provided 3K Custom Assets)
    ...
    if (isAged) {
      for (let key in textureMap) {
        this.textureLoader.load(textureMap[key], (tex) => {
  ...
  _applyGearKinematics(name, curRot, parentCenter, rotOrigin) {
    ...
    // The `curRot` input is already negated to adapt from p5.js.
    // `gearAngle` and `pocAngle` are derived from this negated `curRot`.
    // In p5.js, `ad < 0` meant the indicator was "behind" the contact point.
    // Since we negated the angles, `ad > 0` now means "behind".
  ...
  _applyTextureToMaterial(mat, tex) {
    ...
    // CUSTOM 3K TEXTURE SCALING:
    // With 3k-wide assets, we want a much lower repetition to avoid tiling arcs.
    if (mat.isLargeSurface) {
      newTex.repeat.set(0.005, 0.005);
    } else {
      // For mini gears and pegs, we further reduce repetition to 0.003
      // so a single texture tile easily covers the entire gear.
      newTex.repeat.set(0.003, 0.003);
      // Apply offset to move the texture corner (0,0) away from the geometry center,
      // eliminating the noticeable "4 quadrants" look.
...
// =============================================================================
// Standalone bootstrap — runs when this file is loaded directly (not via host).
// When integrated into CoolweirdClocks, the host app creates the renderer
// and calls init/activate/resize/update directly.
// =============================================================================
let steampunkClock;
let isStarted = false;

window.onload = () => {
  // Only launch automatically if we are standalone.
  if (!isStarted) {
    startSteampunkClock();
  }
};

function startSteampunkClock() {
  isStarted = true;
  steampunkClock = new SteampunkClockRenderer('canvas-container', { standalone: true });
  steampunkClock.init();
  steampunkClock.resize(window.innerWidth, window.innerHeight);
  steampunkClock.activate();
}
